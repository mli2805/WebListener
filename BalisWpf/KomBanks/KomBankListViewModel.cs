...
namespace BalisWpf
{
    public class KomBankListViewModel : PropertyChangedBase
    {
        ...
        public async void Start(IniFile iniFile, IMyLog logFile)
        {
            ...
            _balisSignalRClient = new BalisSignalRClient(iniFile, _logFile);

            foreach (var komBank in _firstPageList)
            {
                var viewModel = await new KomBankViewModel(iniFile, komBank, _logFile, _balisSignalRClient).GetSomeLast();
