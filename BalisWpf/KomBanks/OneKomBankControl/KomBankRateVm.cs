...
namespace BalisWpf
{
    public class KomBankRateVm : PropertyChangedBase
    {
        ...
        public void SetIfExpired(int gap)
        {
            var timeSpan = TimeSpan.FromSeconds(3 * gap);
        ...
        public Tuple<double, double> GetCurrency(string currency)
        {
            return currency switch
            {
                "usd" => new Tuple<double, double>(UsdA, UsdB),
                "eur" => new Tuple<double, double>(EurA, EurB),
                "rub" => new Tuple<double, double>(RubA, RubB),
                "eurusd" => new Tuple<double, double>(EurUsdA, EurUsdA),
                "rubusd" => new Tuple<double, double>(RubUsdA, RubUsdB),
                "rubeur" => new Tuple<double, double>(RubEurA, RubEurB),
                _ => null
            };
        }
