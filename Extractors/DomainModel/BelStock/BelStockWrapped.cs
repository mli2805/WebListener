...
{
    public class BelStockWrapped : INotifyPropertyChanged
    {
        public NbRates NbRates { get; set; }
        private BelStock _belStock;
        public BelStock BelStock
        {
            get { return _belStock; }
            set
            {
        ...
        private string BuildNewBasketString()
        {
            ...
            return $"{newBasket:#,0.0000} " +
                   $"( {((newBasket - NbRates.Basket) * 10000).ToString("+0.00;-0.00")} п / {((newBasket - NbRates.Basket)*100/newBasket).ToString("+0.00;-0.00")}% ) " +
