...
namespace BanksListener
{
    public class Banki24Poller
    {
        ...
        public async Task<BelStock> GetStockAsync()
        {
            try
            {
                var banki24Extractor = new Banki24Extractor();
                var belStock = Parse(await banki24Extractor.GetPageAsync(Url, Encoding.UTF8));
                if (belStock.Usd.Average > -1) belStock.Usd.LastDeal = ParseLastDealRate(await banki24Extractor.GetPageAsync(UrlUsd, Encoding.UTF8));
                if (belStock.Eur.Average > -1) belStock.Eur.LastDeal = ParseLastDealRate(await banki24Extractor.GetPageAsync(UrlEur, Encoding.UTF8));
                if (belStock.Rub.Average > -1) belStock.Rub.LastDeal = ParseLastDealRate(await banki24Extractor.GetPageAsync(UrlRub, Encoding.UTF8));
                return belStock;
            }
            catch (Exception e)
            {
                Console.WriteLine($@"{e.Message} in Bel Stock online parser");
                return new BelStock() {Usd= new BelStockCurrency() {Average = -1}};
            }
        }
        ...
        private BelStock Parse(string webData)
        {
            ...
            var result = new BelStock
            {
                ...
                //TradingState = GetStateTemp(table, out pos),
