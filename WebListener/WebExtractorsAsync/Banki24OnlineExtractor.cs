    ...
    class Banki24OnlineExtractor
    {
        ...
        public async Task<BelStock> GetStockAsync()
        {
            try
            {
                ...
                var belStock = Parse(await banki24Extractor.FetchPageAsync(Url));
                if (belStock.Usd.Average > -1) belStock.Usd.LastDeal = ParseLastDealRate(await banki24Extractor.FetchPageAsync(UrlUsd));
                if (belStock.Eur.Average > -1) belStock.Eur.LastDeal = ParseLastDealRate(await banki24Extractor.FetchPageAsync(UrlEur));
                if (belStock.Rub.Average > -1) belStock.Rub.LastDeal = ParseLastDealRate(await banki24Extractor.FetchPageAsync(UrlRub));
                return belStock;
            }
            catch (Exception e)
            {
